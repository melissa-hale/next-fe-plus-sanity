...
- [x] **2.2** Add `AggregateRating` / `Review` structured data to `LocalBusinessSchema` *(2026-05-02 — deferred: `aggregateRating` block omitted until real reviews exist; adding fake counts risks a Google manual penalty. Revisit when task 3.4 (reviews page) is implemented. **Updated 2026-08-18 — do not revisit for the Google rating.** Phase 9.4 put a visible 5.0 rating in the hero; that must *not* be mirrored as `aggregateRating`. Self-serving review markup on a `LocalBusiness` is against Google's guidelines, and Google's own ratings cannot be republished as your structured data. Only first-party reviews collected on-site (task 3.4) could ever justify this block)*
...
- [x] **9.4** Google rating in the hero *(2026-08-18 — new **`Components/GoogleRating.tsx`**, rendered in `Components/Home.tsx` under the hero CTA: five amber stars + "5.0 on Google", linking out to the Google Business Profile. Under the ask rather than above it, so it reinforces the click instead of delaying the headline. Uses the **text-link** treatment (`hover:text-green-700 hover:underline`), deliberately not the 3d.8 amber lift — it sits directly beneath the green primary button and must not read as a second CTA.*

  ***No review count**, by design: a hardcoded count goes stale as reviews arrive, and Phase 7 copy rule 4 forbids inventing one. The rating alone never rots.*

  ***No `AggregateRating` JSON-LD to match it** — this is the second, better reason to keep 2.2 deferred. Beyond "no real reviews yet", Google's structured data guidelines disallow **self-serving review markup** on a `LocalBusiness` (ratings the business publishes about itself), and ratings collected by Google cannot be re-emitted as your own structured data. The visible, linked-out claim is the honest version of this signal — do not add `aggregateRating` to `StructuredData.tsx` on the strength of the Google rating.*

  ***Open:*** *`GOOGLE_PROFILE_URL` is the Maps name-search URL already used for `hasMap` in `StructuredData.tsx`. It resolves to the profile, but a name search is not stable — swap it for the canonical `g.page/…` short link or the Maps place URL with its CID when that is to hand. There is a `TODO` on the constant.*

  *Home page still builds `○ Static` at 188 B (server component, zero JS). Lighthouse on a prod build: perf **97**, a11y **100**, SEO **100**, LCP **2.4 s**, CLS 0 — `target-size`, `color-contrast`, `heading-order` and `link-name` all pass. The link carries `min-h-[44px]` for target-size, and the "opens in a new tab" text is an appended `sr-only` span rather than an `aria-label`, so the accessible name still begins with the visible text — WCAG 2.5.3 Label in Name, the rule that took the aria-label off this same CTA in 3a.7.)*
...
| ❌ | ~~Add `AggregateRating` to LocalBusiness (2.2)~~ | — | Will not do — self-serving review markup; see 9.4 |
...
| `app/(site)/Components/GoogleRating.tsx` | Five stars + "5.0 on Google", linking to the Google Business Profile. Rendered under the hero CTA in `Home.tsx`. Text-link styling on purpose — it must not read as a second CTA. No count and no `AggregateRating` JSON-LD; see Phase 9.4 before changing either |
| `app/(site)/Components/ClosingCTA.tsx` | The block a page ends on — one green primary CTA into `/contact`, phone repeated below as a text link, not a second button. Shared by the home page (`tint`), `/gallery` (`tint`) and `/service-area` (untinted, since the town list above it is tinted); only `heading` and `body` differ. **Always a top-level sibling of the other sections, never nested inside one** — it renders its own `Section`, so nesting it confines the tint band to that parent's max-width column instead of the viewport. On `/gallery` this means it belongs in `[slug]/page.tsx` after `<Header>`, not inside `[slug]/Gallery.tsx`. Carries the 3d.8 primary CTA string verbatim — edit it here, not at the call sites |
